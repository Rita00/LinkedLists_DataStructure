...
void imprime_lista_utilizadores(lista_utilizadores *lista) {
    while (lista->next != NULL) {
        lista = lista->next;
        printf("%d\t%s\t%s\t%d/%d/%d\t%d\n\n", lista->pessoa->id, lista->pessoa->nome_utilizador, lista->pessoa->morada,lista->pessoa->nascimento.dia,lista->pessoa->nascimento.mes,lista->pessoa->nascimento.mes,lista->pessoa->telefone);
    }
}
    ...
    while(opcao != 0){
        ...
        switch(opcao){
            case 1:
                ...
                inserir_utilizador(pesquisa_local(BDlocais,id_local)->util,pesquisa_utilizador(BDutilizadores,id_util)); /*NAO TESTADO*/
                break;
            case 2:
                printf("Qual o id do local que deseja?\n");
                scanf("%d",&id_local);
    ...
    while(opcao != 0){
        ...
        switch(opcao){
            case 1:
                ...
                inserir_utilizador(pesquisa_PDI(BDPDIs, id_PDI)->util,pesquisa_utilizador(BDutilizadores, id_util)); /*NAO TESTADO*/
                break;
            case 2:
                printf("Qual o id do PDI que deseja?\n");
                scanf("%d",&id_PDI);
